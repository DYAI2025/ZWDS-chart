...
## AMD-003 geocode pin extended n=1 -> n=3 (2026-07-18)

Extended scripts/amd003-pin.mjs to pin MULTIPLE real geocode profiles (geocode-<slug>.json) and captured 3 live from real FuFirE: geocode-berlin.json (1 result, unambiguous 200 single-match branch, upstream confidence), geocode-shanghai.json (5) and geocode-taipei.json (3) (ambiguous 422 candidate branch, timezone derived offline via tz-lookup). Migrated the singular geocode.json away. tests/reality/geocode-pin.test.mjs now globs geocode-*.json, asserts each conforms to the contract + every result has a valid IANA timezone, AND enforces diversity: the set must cover both a single-match (n==1) and a multi-candidate (n>1) response, so the single-match parser branch is no longer unexercised. Secret scan on all pinned files: 0 hits. Full gates green (vitest 92 passed, 0 skipped; eslint/arch/tsc 0). This retires the n=1 single-sample fragility for the geocode boundary.

## Source-governance elevation layer built (REQ-021) — ships inert (2026-07-18)

User chose "verify-layer + report-elevation". Built server/governance/{attestation,verify}.mjs: hash-pinned reviewer-attestation loader + fail-closed elevation guard threaded through normalizeRaw so report.calculation.sourceStatus (the single AMD-002 authority switch) is raised SOURCE_NEEDED -> SOURCE_REVIEWED ONLY by a named attestation whose reviewedRulesetSha256 exactly matches the live ruleset digest (same id+version, no BLOCKED, no MISMATCH). Wired /api/zwds/ruleset-status (governance.reviewedBy) + /api/config-status (rulesetGovernance). Fixture ruleset_sha256 = 3f5cef4a...47f (real, matchable). 12 governance tests (real modules + TEST-ONLY pinned attestation + supertest); 6-vector adversarial sweep = 0 authority bypasses. Full gates green (vitest 104, tsc/eslint/arch/build 0). Ships DISABLED: no attestation in prod -> every chart stays not-authoritative, unchanged. REQ-021 -> integration-fake. Real authoritative status still needs a genuine NAMED source-governance reviewer (cannot be fabricated).

## Catalogue-digest governance built (REQ-021 extension) — 2026-07-18

Added a deterministic catalogue content digest (CATALOG_CONTENT_SHA256=fc619840...773d over canonicalCatalogueString of the full 8-array catalogue) with a build-time drift guard, plus fail-closed catalogueGovernanceStatus() that reports SOURCE_REVIEWED only when a reviewed pin (CATALOG_SHA256, null by default, never fabricated) exactly matches the content digest. Surfaced on the report Method page (catalogue id/version/digest/status). 5 unit cases; full gates green (vitest 109/0-skip, tsc/eslint/arch/build 0). Ships SOURCE_NEEDED: no reviewer has pinned a reviewed catalogue digest. Retires the KNOWN_LIMITATIONS "Catalogue digest" gap to content-pinned + governance-ready.
