...
describe('canonical FuFirE golden fixture', () => {
  it('uses canonical placement truth and required anchors', () => {
    ...
    expect(fixture.chart.five_elements_bureau).toEqual({ id: 'FIRE_6', number: 6 });
    expect(fixture.resolution.calendar.late_zi_applied).toBe(true);
    expect(fixture.chart.palaces).toHaveLength(12);
    expect(fixture.chart.decadal_limits).toHaveLength(12);
    const palaces = new Map(fixture.chart.palaces.map((palace) => [palace.palace_id, palace]));
  ...
  it('binds transformations to concrete original placement ids', () => {
    ...
    expect(placements.get('natal:TAI_YANG')).toMatchObject({ palace_id: 'GUAN_LU', transformation_types: ['HUA_JI'] });
    expect(placements.get('natal:WU_QU')).toMatchObject({ palace_id: 'TIAN_ZHAI', transformation_types: ['HUA_KE'] });
    expect(placements.get('natal:PO_JUN')).toMatchObject({ palace_id: 'TIAN_ZHAI', transformation_types: ['HUA_QU'] });
    expect(placements.get('natal:LIAN_ZHEN')).toMatchObject({ palace_id: 'XIONG_DI', transformation_types: ['HUA_LU'] });
  });
  ...
  it('normalizes ids, provenance, warnings and trace losslessly', () => {
    ...
    expect(report.transformations.find((item) => item.transformationId === 'HUA_QU')).toMatchObject({ placementId: 'natal:PO_JUN', starId: 'PO_JUN', palaceId: 'TIAN_ZHAI' });
    expect(report.provenance[0].recordId).toBe(fixture.provenance[0].record_id);
    expect(report.quality.warnings[0].code).toBe('RULESET_HASHES_UNAVAILABLE');
